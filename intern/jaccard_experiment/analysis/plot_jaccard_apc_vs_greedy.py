"""
Plot: APC beta vs Mean Jaccard Similarity (APC vs Greedy) for InternVL3-8B.
"""
...
ax.set_title("APC Converges to Greedy Search as β Increases\n"
             "InternVL3-8B · LLaVA-Bench (60 questions)",
             fontsize=13, fontweight="bold", pad=12)

ax.set_xlim(-0.02, 1.02)
# Unlike the Qwen plot, the y-limits are derived from the data rather than
# hard-coded to (0.30, 0.85) — InternVL3's sampling-vs-greedy spread is not
# known in advance, and a fixed window could clip the curve.
lo = float(df["mean_jaccard"].min())
hi = float(df["mean_jaccard"].max())
