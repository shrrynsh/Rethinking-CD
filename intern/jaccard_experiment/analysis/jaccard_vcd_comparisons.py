...
All comparisons are restricted to the question IDs present in both files, so a
partial/interrupted VCD run still yields valid numbers (the n column reports
how many questions each comparison actually covered).

Model: InternVL3-8B (Qwen2.5-7B LLM backbone → Qwen2TokenizerFast BPE)
...
from common import (APC01_FILE, EXPERIMENT, GREEDY_FILE, MODEL_PATH,
                    SAMPLE_FILE, VCD_FILE, jaccard, load_jsonl)
